...
def G(X, Y, A, B, N1, N2):
    ...
    with open(file_name, "a+") as fileVar:
        output = str(k) + " " + str(round(g, 5)) + " " + str(round(dg, 5)) + " " + str(round(x, 5)) + " " + \
                 ...
                 str(round(eff_2, 5)) + " " + str(round(deff2, 5)) + " " + str(round(NRF, 5)) + " " + str(round(dNRF, 5)) + " " + str(round(alpha, 5))+ "\n"
        fileVar.write(output)

    return g

q_APD = np.genfromtxt('C:/Users/Пользователь/Desktop/Лаба/калибровки/Продолжение работы по калибровки аналоговых детекторов/Взаимные калибровки/Меньше шаг по мощности/m4.1t35s900O.txt', 'float64')
q_PMT = np.genfromtxt('C:/Users/Пользователь/Desktop/Лаба/калибровки/Продолжение работы по калибровки аналоговых детекторов/Взаимные калибровки/Меньше шаг по мощности/m4.1t35s900F.txt', 'float64')
for k in range(0, 1, 1):
    ...
    with open('C:/Users/Пользователь/Desktop/3.txt', "w+") as file:
        ...
        my_file1 = "C:/Users/Пользователь/Desktop/Лаба/калибровки/Продолжение работы по калибровки аналоговых детекторов/Взаимные калибровки/Меньше шаг по мощности/m4.1t35s900O.txt"
    ...
    # green
    X = delete_noize_new(my_file1, 0, 99999, 0, 0, 0, 0, 'new_file1.txt')
    # orange
    Y = delete_noize_new(my_file2, 0, 99999, 0, 0, 0, 0, 'new_file2.txt')
    g = G(X, Y, 1361, 99, 1598.60743, 2708.3892) # последние два числа это найденый ранее уровень фона
    # print(j, g)
    print(k, g)
